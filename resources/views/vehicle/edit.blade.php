...
<div class="vehicle-edit-dual-grid">
    ...
    {{-- CONTEÚDO --}}
        ...
        <div class="edit-card vehicle-full-card vehicle-fuel-products">
            ...
            <div class="procedures-grid">@foreach($fuelProducts as $product)<label class="procedure-pill"><input type="checkbox" name="fuel_product_ids[]" value="{{ $product->id }}" @checked(in_array($product->id, old('fuel_product_ids', $vehicle->fuelProducts->pluck('id')->all())))><span>{{ $product->name }}</span></label>@endforeach</div>
