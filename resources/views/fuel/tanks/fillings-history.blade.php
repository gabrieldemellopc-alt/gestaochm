...
<section class="fuel-panel"><div class="fuel-table-wrap"><table class="fuel-table"><thead><tr><th>Data / veículo</th><th>Origem / produto</th><th>Quantidade</th><th>KM</th><th>Responsável</th><th>Status</th><th></th></tr></thead><tbody>
@forelse ($fillings as $filling)
<tr class="{{ $filling->cancelled_at ? 'is-cancelled' : '' }}"><td>{{ $filling->filled_at?->format('d/m/Y H:i') }}<br>{{ $filling->vehicle?->name }} · {{ $filling->vehicle?->plate }}</td><td>{{ $filling->source_label }}<br><small>{{ $filling->location_label }} · {{ $filling->product?->name }}</small></td><td>{{ number_format((float) $filling->quantity_liters, 3, ',', '.') }} L @if ($fuelPermissions['view_costs'])<small>R$ {{ number_format((float) $filling->total_cost, 2, ',', '.') }}</small>@endif</td><td class="fuel-history-reading"><strong>{{ $filling->vehicle_km !== null ? rtrim(rtrim(number_format((float) $filling->vehicle_km, 2, ',', '.'), '0'), ',') : '—' }}@if ($filling->vehicle_km !== null) km @endif</strong></td><td>{{ $filling->responsible?->name ?: '—' }}</td><td><span class="fuel-history-status {{ $filling->cancelled_at ? 'is-cancelled' : 'is-complete' }}">{{ $filling->cancelled_at ? 'Cancelado' : 'Realizado' }}</span>@if ($filling->cancelled_at)<small>{{ $filling->cancel_reason }} · {{ $filling->canceller?->name }}</small>@endif</td><td>@if (! $filling->cancelled_at && $fuelPermissions['cancel'])<button type="button" class="fuel-secondary-action fuel-cancel-action" x-on:click="cancelId = {{ $filling->id }}">Cancelar</button>@endif</td></tr>
@empty
