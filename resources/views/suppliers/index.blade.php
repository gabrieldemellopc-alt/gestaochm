...
<div class="suppliers-page" x-data="supplierAdmin()" x-effect="document.body.classList.toggle('suppliers-modal-open', modal.open)">
    ...
    <section class="suppliers-surface">
        ...
        @forelse($items as $supplier)<tr>
            ...
            <td><div class="suppliers-actions"><button type="button" @click='openEdit(@js(["id"=>$supplier->id,"trade_name"=>$supplier->trade_name,"legal_name"=>$supplier->legal_name,"document"=>$supplier->formattedDocument(),"active"=>(bool)$supplier->active,"aliases"=>$supplier->aliases->pluck("alias")->values()]))'><i class="bi bi-pencil"></i> Editar</button><button type="button" class="suppliers-status-toggle" :aria-pressed="@js((bool) $supplier->active)" @click='toggle(@js(["id"=>$supplier->id,"trade_name"=>$supplier->trade_name,"legal_name"=>$supplier->legal_name,"document"=>$supplier->formattedDocument(),"active"=>(bool)$supplier->active,"name"=>$supplier->displayName(),"aliases"=>$supplier->aliases->pluck("alias")->values()]))'><span class="suppliers-status-toggle__track {{ $supplier->active ? 'is-active' : '' }}"><span></span></span><span>{{ $supplier->active ? 'Ativo' : 'Inativo' }}</span></button></div></td>
        </tr>@empty
