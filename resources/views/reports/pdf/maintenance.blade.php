...
<body>
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi-box">
            ...
            <div class="kpi-label">Ordens de manutenção</div>
    ...
    </div>    <div class="procedure-stats-section">
    ...
    <div class="vehicle-cost-section">
        ...
        <table class="vehicle-cost-table">
            ...
            <thead>
                <tr>
                    <th>Ordem</th><th>Entrada</th><th>Saída</th><th>Veículo</th><th>Placa</th><th>Composição</th><th>Status</th><th>Valor</th>
                </tr>
            </thead>
            <tbody>
                @foreach($maintenances as $maintenance)
                    <tr>
                        ...
                        <td>
                            ...
                            @endphp
                            @if($services->isNotEmpty())<div>{{ $services->count() === 1 ? 'Serviço' : 'Serviços' }}<br>{{ $services->count() }} · R$ {{ number_format($services->sum('total_cost'), 2, ',', '.') }}</div>@endif
                            @if($extraCosts->isNotEmpty())<div>{{ $extraCosts->count() === 1 ? 'Custo avulso' : 'Custos avulsos' }}<br>{{ $extraCosts->count() }} · R$ {{ number_format($extraCosts->sum('amount'), 2, ',', '.') }}</div>@endif
                        </td>
                        <td>{{ ['closed' => 'Encerrada', 'open' => 'Aberta', 'cancelled' => 'Cancelada'][$maintenance->workflow_status] ?? ($maintenance->workflow_status ?? '-') }}</td>
                        <td><strong>R$ {{ number_format($maintenance->total_cost, 2, ',', '.') }}</strong></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
