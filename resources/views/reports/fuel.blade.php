            ...
            <div class="tire-compact-list">
                @forelse($latest_fillings as $filling)
                    <div class="tire-compact-item">
                        ...
                        <span>{{ $formatDate($filling->filled_at) }} | {{ $filling->source_label }} - {{ $filling->location_label }} | {{ $filling->product?->name ?? '-' }} | {{ $liters($filling->quantity_liters) }} | {{ $money($filling->reporting_total_cost) }}</span>
                    </div>
                @empty
