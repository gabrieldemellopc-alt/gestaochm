@extends('layouts.app')
@push('styles')<link rel="stylesheet" href="{{ asset('css/pages/reports-financial.css') }}?v=1"><link rel="stylesheet" href="{{ asset('css/pages/reports-financial-stock-purchases.css') }}?v=1">@endpush
@section('content')
...
<section class="chm-financial-report__stock-purchases"><div class="chm-financial-report__stock-purchases-heading"><span>MOVIMENTAÇÃO FINANCEIRA / ESTOQUE</span><h2>Aquisições de estoque</h2><p>Compras e entradas de materiais realizadas no período.</p></div><div class="chm-financial-report__stock-purchases-metric"><i class="bi bi-box-seam"></i><strong>R$ {{ number_format($stock_purchases_total,2,',','.') }}</strong><small>{{ $stock_purchase_entries_count }} {{ $stock_purchase_entries_count === 1 ? 'entrada' : 'entradas' }} de compra no período</small></div></section>
<section class="chm-financial-report__stock-purchases-note"><i class="bi bi-info-circle"></i><p>Este valor representa aquisições de estoque e não é somado ao custo operacional. Os materiais são apropriados como custo quando utilizados.</p></section>
