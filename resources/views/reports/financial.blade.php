...
<section class="chm-financial-report__stock-purchases"><div class="chm-financial-report__stock-purchases-heading"><span>MOVIMENTAÇÃO FINANCEIRA / ESTOQUE</span><h2>Aquisições de estoque</h2><p>Compras destinadas à reposição ou formação do estoque disponível.</p></div><div class="chm-financial-report__stock-purchases-metric"><i class="bi bi-box-seam"></i><strong>R$ {{ number_format($stock_purchases_total,2,',','.') }}</strong><small>{{ $stock_purchase_entries_count }} {{ $stock_purchase_entries_count === 1 ? 'entrada' : 'entradas' }} de compra no período</small></div></section>
<section class="chm-financial-report__stock-purchases-note"><i class="bi bi-info-circle"></i><p>Compras diretas realizadas para uso imediato em manutenção não entram neste indicador, pois já são apropriadas no custo da OM.</p></section>
