...
class VehicleDossierReportService
{
    ...
    private function maintenanceRow(MaintenanceRecord $maintenance, array $context): array
    {
        ...
        return [
            ...
            'materials' => $maintenance->materialUsages->sortBy('created_at')->map(fn ($usage) => [
                'name' => $usage->stockItem?->name ?? 'Item de estoque',
    ...
    private function fuelFillingRow(FuelFilling $filling): array
    {
        return [
            ...
            'tank' => $filling->tank,
            'source' => $filling->resolved_source,
            'source_label' => $filling->source_label,
            ...
            'responsible_name' => $filling->responsible?->name ?? 'Não informado',
            'registered_by_name' => $filling->responsible?->name ?? 'Não informado',
            'responsible_user_name' => $filling->responsible?->name ?? 'Não informado',
            'notes' => $filling->notes,
    ...
    private function cancelledFuelFillings(array $context, Vehicle $vehicle, array $filters): Collection
    {
        return FuelFilling::query()
            ...
            ->map(fn (FuelFilling $filling) => [
                ...
                'record_label' => ($filling->product?->name ?? $filling->tank?->product?->name ?? 'Produto') . ' - ' . $filling->location_label,
                'date' => $filling->filled_at,
