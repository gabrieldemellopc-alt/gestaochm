...
class PreventiveService
{
    public static function getVehicleAlerts($vehicle)
    {
        ...
        foreach ($procedures as $procedure) {
            $latest = MaintenanceRecordItem::query()
                ->where('procedure_id', $procedure->id)
                ->whereHas('maintenanceRecord', function ($query) use ($vehicle) {
                    $query
                        ->where('vehicle_id', $vehicle->id)
                        ->whereNull('cancelled_at')
                        ->whereIn('workflow_status', ['open', 'closed']);
                })
                ->latest('performed_at')
                ->latest('id')
                ->first();
        ...
        */
        ...
        |--------------------------------------------------------------------------
        */
        if ($vehicle->km_control_enabled && $kmReferenceDate) {
            $daysWithoutKmUpdate =
        ...
        */
        ...
        |--------------------------------------------------------------------------
        */
        if ($vehicle->hours_control_enabled && $hoursReferenceDate) {
        ...
        */
        ...
        $expectedPositions = $vehicle->tirePositions()
            ->where('active', true)
            ->pluck('code')
            ->values();

        $activeInstalledPositions = TireInstallation::query()
        ...
        $missingPositions = $expectedPositions
            ->diff($activeInstalledPositions)
            ->values();

        if ($expectedPositions->count() > 0 && $missingPositions->count() > 0) {
            $alerts[] = [
                'status' => 'warning',
                'procedure' => 'Controle de pneus',
                'message' => 'Pneus ausentes ('.$missingPositions->count().' posições)'
                ];
        }
                ...
                if ($daysWithoutTireMeasurement >= 60) {
                    ...
                    $alerts[] = [
                        ...
                        'message' =>
                            'Pneu ' . $tire->code .
                            ' (' . $installation->position_code . ')' .
                            ' está há ' . $daysWithoutTireMeasurement  .
