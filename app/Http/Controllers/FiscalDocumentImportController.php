...
use App\Models\{FiscalDocument,FiscalDocumentItem,StockCategory,StockItem};
use App\Services\{ActiveContextService,AuditLogService,StockEntryService};
use App\Services\FiscalDocuments\{DanfePdfParser,FiscalDocumentItemMatcher,NfeXmlParser};
...
class FiscalDocumentImportController extends Controller
{
    ...
    public function parse(Request $r, DanfePdfParser $pdfParser, NfeXmlParser $xmlParser, FiscalDocumentItemMatcher $matcher)
    {
    ...
    public function confirm(Request $r, StockEntryService $entries)
    {
        ...
        $document=DB::transaction(function() use($v,$draft,$u,$location,$key,$entries){
            $final="fiscal-documents/{$u->tenant_id}/".now()->format('Y').'/'.Str::uuid().'.'.$draft['extension']; Storage::disk('local')->move($draft['path'],$final);
            $doc=FiscalDocument::create(['tenant_id'=>$u->tenant_id,'division_id'=>session('active_division_id'),'location_id'=>$location->id,'created_by'=>$u->id,'number'=>$v['number'],'series'=>$v['series']??null,'access_key'=>$key?:null,'issued_at'=>$v['issued_at']??null,'supplier_name'=>$v['supplier_name'],'supplier_cnpj'=>preg_replace('/\D/','',$v['supplier_cnpj']??''),'products_total'=>$v['products_total']??null,'total_amount'=>$v['total_amount']??null,'pdf_path'=>$final]);
            foreach($v['items'] as $line){ if($line['action']==='ignore') continue; $created=false;
                ...
                $movement=$entries->record($item,['quantity'=>$line['quantity'],'total_cost'=>$line['total_value'],'invoice_number'=>trim($v['number'].' / '.$v['series'],' /'),'supplier_name'=>$v['supplier_name'],'description'=>'Importação validada de NF-e','moved_at'=>$v['issued_at']??now(),'fiscal_document_id'=>$doc->id],['summary'=>'Movimentação criada por importação validada de NF-e.']);
