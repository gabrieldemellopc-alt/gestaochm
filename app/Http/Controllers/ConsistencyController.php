...
class ConsistencyController extends Controller
{
    public function index(Request $request, ActiveContextService $context)
    {
        ...
        $locations=$context->availableLocations($user,$division->id); $activeLocation=$context->activeLocation($user); if(!$activeLocation)return redirect()->route('portal')->with('warning','Selecione uma unidade para consultar a Central de Consistência.'); $locationId=(int)$activeLocation->id;
        $query=DataConsistencyAlert::query()->with(['reviewer:id,name','location:id,name'])->where('tenant_id',$user->tenant_id)->where('division_id',$division->id)->where('location_id',$locationId);
        ...
        $base=DataConsistencyAlert::query()->where('tenant_id',$user->tenant_id)->where('division_id',$division->id)->where('location_id',$locationId);
    ...
    private function authorizeAlert(Request $request, DataConsistencyAlert $alert):void { $user=$request->user(); $context=app(ActiveContextService::class); $activeLocation=$context->activeLocation($user); abort_unless($activeLocation && (int)$alert->tenant_id===(int)$user->tenant_id && (int)$alert->division_id===(int)session('active_division_id') && (int)$alert->location_id===(int)$activeLocation->id,403); }
